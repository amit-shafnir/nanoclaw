...
describe('runCodexInstallCheck', () => {
  ...
  it('blocks terminal setup when the Codex payload is incomplete', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'nanoclaw-codex-install-check-'));
    const driver = new FakeDriver('terminal');
    try {
      await expect(runCodexInstallCheck(driver, root)).rejects.toThrow(/Codex provider is not fully installed/);
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
