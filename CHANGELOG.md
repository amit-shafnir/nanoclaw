...
- [BREAKING] **Agent templates are now Agent Plugins 1.0.0 directories.** `plugin.json` replaces `context/instructions.md` as the required file and discovery marker; MCP servers move to a spec-shaped `mcp.json` (declared `type`, optional `headers` on remote servers); persona, extra context, and tasks move under the `ai.nanoco.nanoclaw/` extension dir. Templates are portable to other plugin clients, and any conformant third-party plugin stamps as a new agent group. Stamping copies the whole plugin to a read-only `plugins/<name>` mount with a writable `plugin-data/<name>`, injects `PLUGIN_ROOT`/`PLUGIN_DATA` for plugin stdio servers, rejects symlinked or over-sized templates, and rejects credential-shaped `env`/`headers` values (`"placeholder"` always passes). **Migration:** re-fetch templates from the registry (the pre-plugin layout now fails with a migration error); local custom templates need `plugin.json`, `mcp.json`, and the extension-dir moves — see [docs/templates.md](docs/templates.md).
