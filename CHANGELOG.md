...
- **Typing refresh cadence is per channel.** An adapter may declare its platform's typing-indicator lifetime (`typingTimeoutMs`); the host re-fires `setTyping` 20% before it would expire. Undeclared adapters keep today's 4 s. WhatsApp Cloud declares 25 s.
