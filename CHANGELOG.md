...
- **The 3-day pnpm release-age gate is now enforced.** `minimumReleaseAge` was nested under `pnpm:` in `pnpm-workspace.yaml`, where pnpm ignores it. `pnpm add` of an exact pin younger than 3 days now refuses to resolve, ranges fall back to the newest mature version (`minimumReleaseAgeExclude` is the escape hatch); frozen installs are unaffected.
