/**
 * Guardrails config loader (host side).
 *
 * Per-group config lives at groups/<folder>/guardrails/guardrails.json with
 * optional sidecar files (phrases_file) in the same directory. The directory
 * is RO-mounted into the agent container; the host reads it directly from
 * GROUPS_DIR.
 *
 ...
 *
 * The loader is the no-op fast path for unconfigured groups: results
 * (including 'absent' and 'invalid') are cached and revalidated by mtime at
 * most once per CACHE_TTL_MS, so an unconfigured group costs one Map lookup
 * per message and a fixed config is picked up within ~5s.
 *
 * A near-identical copy lives in container/agent-runner/src/guardrails/
 * (different base dir + logger). Keep edits in sync.
 */
import fs from 'fs';
...
export function guardrailsDir(folder: string, baseDir: string = GROUPS_DIR): string {
  return path.join(baseDir, folder, 'guardrails');
}

/** Load (with caching) the guardrails config for an agent group folder. */
export function loadGuardrailsConfig(folder: string, baseDir: string = GROUPS_DIR): GuardrailsLoadResult {
  const dir = guardrailsDir(folder, baseDir);
  const now = Date.now();
...
function loadFresh(dir: string): { result: GuardrailsLoadResult; files: CacheEntry['files'] } {
  ...
  try {
    ...
    log.error('Guardrails config INVALID — all messages for this group will be blocked until fixed', {
      configPath,
      error,
    });
